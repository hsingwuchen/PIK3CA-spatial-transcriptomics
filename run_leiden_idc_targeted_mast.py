"""
Visium HD Human Breast Cancer Fixed Frozen (IDC) -- Leiden clustering pipeline
Dataset : Visium HD CytAssist Gene Expression - Human Breast Cancer Fixed Frozen
          IDC, Female 76yr, Space Ranger v3.1.1

Outputs:
  VisiumHD_FF_leiden_clusters.png
  VisiumHD_FF_leiden_dotplot.png
  VisiumHD_FF_PIK3CA_GS_comparison.png
  VisiumHD_FF_4panel_fixed.png
  VisiumHD_FF_lipid7_panel.png
  lipid_proximity_stats_full.tsv
  VisiumHD_FF_proximity_summary.png

======================================================
COLAB SETUP — 在最上面另外建一格 cell 執行這兩段：
------------------------------------------------------
from google.colab import drive
drive.mount('/content/drive')
!pip install -q scanpy leidenalg
------------------------------------------------------
然後把下面 === PATH SETTINGS === 區塊的路徑
改成你的 Google Drive 路徑再跑本腳本。
======================================================
"""

# ============================================================
# PATH SETTINGS  ← 只需要改這裡
# ============================================================
# 本機執行時用這組：
H5   = "D:/VisiumHD_BC_IDC_FF/feature_slice.h5"

# Colab 執行時把上面三行換成（先把檔案上傳到 Drive）：
# H5   = "/content/drive/MyDrive/VisiumHD/data/feature_slice_idc.h5"
# SIGF = "/content/drive/MyDrive/VisiumHD/pik3ca_gs_with_symbols.csv"
# OUT  = "/content/drive/MyDrive/VisiumHD/output_idc"
# ============================================================
...
H5 = os.environ.get("H5", "/content/drive/MyDrive/VisiumHD/IDC/feature_slice.h5")
...
OUT_ROOT = os.environ.get("TARGETED_MAST_OUT_ROOT", "/content/drive/MyDrive/VisiumHD/IDC/targeted_mast_compare")
...
# Read MPP from metadata if available, otherwise use default
MPP = 0.5468406924593424
try:
    mpp_meta = meta.get('pixel_size_um', None)
    if mpp_meta and float(mpp_meta) > 0:
        MPP = float(mpp_meta)
except Exception:
    pass
print(f"  MPP   : {MPP:.6f} um/px")
...
if not cancer_clusters:
    print("  WARNING: No Cancer cluster found. Using top-1 cluster by CA score.")
    cancer_clusters = sorted_by_ca[:1]
    for c in cancer_clusters:
        cluster_to_type[c] = 'Cancer'

print(f"\n  Cancer clusters  : {cancer_clusters}")
if mast_clusters:
    print(f"  Leiden mast-like cluster(s) for QC only: {mast_clusters}")
else:
    top_mc_c = sorted_by_mc[0]
    top_mc_score = cluster_scores[top_mc_c]['Mast']
    print(f"  No Leiden mast cluster used. Top cluster by MC score is C{top_mc_c} ({top_mc_score:.3f}); "
...
fig.suptitle(
    "Leiden Clustering  |  Visium HD Human Breast Cancer Fixed Frozen (IDC, Female 76yr)  |  Space Ranger v3.1.1\n"
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 0b: Marker dotplot per cluster
# ─────────────────────────────────────────────────────────────────────────────
...
fig.suptitle(
    f'PIK3CA-GS Score  |  Visium HD Human Breast Cancer Fixed Frozen (IDC)  |  Space Ranger v3.1.1  |  '
...
fig.suptitle(
    f"Visium HD Human Breast Cancer Fixed Frozen (IDC)  |  8 µm bins  |  Space Ranger v3.1.1\n"
...
fig.suptitle(
    ...
    "Visium HD Human Breast Cancer Fixed Frozen (IDC)  |  8 µm bins\n"
    "Red = Mast-cell-enriched bins  |  * = OR > 1 & p < 0.05 vs gene-low/negative cancer bins  |  Space Ranger v3.1.1",
...
ax.set_xlabel("Odds Ratio (95% CI)  within 50 µm of mast cell", fontsize=10)
ax.set_yticks([]); ax.set_xlim(-0.05, None)
ax.set_title(f"A   Forest Plot: Gene-high vs Gene-low Cancer Bins\n(Fisher's Exact, 50 µm, Q{int(LIPID_POS_QUANTILE*100)} threshold)",
...
ax2.set_ylabel("Median nearest mast cell distance (µm)", fontsize=10)
...
fig.suptitle(
    ...
    "Visium HD Human Breast Cancer Fixed Frozen (IDC)  |  Space Ranger v3.1.1  |  Mast: targeted TPSAB1/TPSB2/CPA3 mask",
    fontsize=11, fontweight='bold', y=1.02)
plt.tight_layout()
...
for fname in output_files:
    status = "OK" if os.path.exists(f"{OUT}/{fname}") else "MISSING"
