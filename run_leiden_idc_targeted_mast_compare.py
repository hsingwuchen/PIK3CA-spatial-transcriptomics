"""
Run the IDC targeted mast-cell pipeline twice so the two mast definitions can
...
Outputs:
  /content/drive/MyDrive/VisiumHD/IDC/targeted_mast_compare/high_confidence/
  /content/drive/MyDrive/VisiumHD/IDC/targeted_mast_compare/sensitive/
"""
...
SCRIPT = Path(__file__).with_name("run_leiden_idc_targeted_mast.py")
DRIVE_ROOT = Path("/content/drive/MyDrive/VisiumHD")
OUT_ROOT = DRIVE_ROOT / "IDC" / "targeted_mast_compare"
H5_PATH = DRIVE_ROOT / "IDC" / "feature_slice.h5"
...
def run(mask_name: str) -> None:
    ...
    print("\n" + "=" * 80, flush=True)
    print(f"Running IDC targeted mast pipeline: {mask_name}", flush=True)
...
def main() -> None:
    ...
    print("\n" + "=" * 80)
    print("IDC comparison run complete.")
    for mask_name in MASKS:
